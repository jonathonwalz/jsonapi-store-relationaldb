...
SqlStore.prototype.initialise = function (resourceConfig) {
  ...
  self.sequelize = self.config.baseModel.sequelize;
  self.baseModel = self.config.baseModel;
  self.relations = { }
  self.relationArray = [ ]
  Object.keys(self.baseModel.associations).forEach(function(key) {
    var relation = self.baseModel.associations[key]

    self.relations[key] = relation
    self.relationArray.push(relation)
  })
...
SqlStore.prototype.populate = function (options, callback) {
  ...
  async.eachSeries(self.resourceConfig.examples.reverse(), function(exampleJson, ecb) {
...
SqlStore.prototype._fixObject = function(json) {
  var self = this
  self.relationArray.filter(r => r.associationType !== 'HasMany').forEach(function (relation) {
    if (relation.isMultiAssociation) {
      json[relation.as] = (json[relation.as] || []).map(r => Object.assign({type: relation.target.name}, {id: r.id}))
    } else {
      json[relation.as] = json[relation.foreignKey]
        ? Object.assign({type: relation.target.name}, {id: json[relation.foreignKey]})
        : null
      delete json[relation.foreignKey]
    }
  })
...
SqlStore.prototype._generateSearchIncludes = function(relationships = {}) {
  ...
  var searchIncludes = self.relationArray.reduce(function(partialSearchIncludes, relation) {
    var matchingValue = relationships[relation.as]
    if (matchingValue || relation.isMultiAssociation) {
      partialSearchIncludes.findAll.push(relation)
    }
...
SqlStore.prototype._clearAndSetMany = function (relation, prop, theResource, t, taskCallback) {
  theResource[relation.accessors['set']](prop.map(p => p.id), t).asCallback(taskCallback)
}

SqlStore.prototype._clearAndSetOne = function (relation, prop, theResource, t, taskCallback) {
  theResource[relation.accessors['set']](prop && prop.id, t).asCallback(taskCallback)
}
  ...
  Object.keys(self.relations).forEach(function (relationName) {
    ...
    tasks[relationName] = function(taskCallback) {
      if (prop instanceof Array) {
        self._clearAndSetMany(relationModel, prop, theResource, t, taskCallback)
      } else {
        self._clearAndSetOne(relationModel, prop, theResource, t, taskCallback)
      }
    }
  })
...
SqlStore.prototype.find = function (request, callback) {
  ...
  self.baseModel.findOne({
    ...
    include: self.relationArray.filter(r => r.isMultiAssociation || request.params.relation === r.as)
  ...
  self._dealWithTransaction(finishedCallback, function (t, finishTransaction) {
    self.baseModel.findOne({
      ...
      self._clearAndSetRelationTables(theResource, partialResource, t, function (err) {
        if (err) return finishTransaction(err)
        var relationKeys = self.relationArray.map(r => r.as)

        theResource.update(partialResource, {
          transaction: t.transaction,
          fields: Object.keys(partialResource).filter(key => relationKeys.indexOf(key) === -1)
        }).asCallback(function (err3) {
